import json 
from functions import get_match, all_party_size_one, gold_total_per_minute, xp_total_per_minute, wards_placed_by_minute, towers_destroyed_by_minute, get_first_blood, all_players_divine
import time 
import os
...
for match_id in match_ids:
    processed_match = {}
    match_id_call = match_id['match_id']
    time.sleep(1)
    if counter < 0:
        counter += 1
        continue

    parsed_match = get_match(BASE_URL=BASE_URL, match_id=match_id_call)    
    ...
    if parsed_match['duration'] < 660:
        ...
        continue
    elif not all_players_divine(parsed_match):
        print("trash tier match")
        continue
